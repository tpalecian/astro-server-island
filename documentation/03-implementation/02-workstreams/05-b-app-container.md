...
- **Approach (one sentence):** Wire the app to the CMS by resolving `@rotate/cms` to the service-dato package, add one page (`index.astro`) that calls `getHomepage()` in frontmatter and passes the result as props to one module (`HomeHero.astro`), which renders props only with no fetch or CMS imports.
- **Exact file name(s) and paths:** Edit `apps/website/astro.config.mjs` and/or `apps/website/tsconfig.json` (alias); create `apps/website/src/pages/index.astro`; create `apps/website/src/components/modules/HomeHero.astro`. No other app files.
...
## 4. Contract / API
- **Alias:** App resolves `@rotate/cms` to packages/service-dato (Vite/TS). Import only getHomepage, GetterOptions, types from `@rotate/cms`. **Page:** `src/pages/index.astro` — frontmatter: `await getHomepage({ token })`, pass result as props to module(s). **Module:** e.g. `src/components/modules/HomeHero.astro` — props only (title, heroWords, heroVideo, etc.); no fetch, no CMS imports. Token from `import.meta.env.DATOCMS_API_KEY` in page.
...
2. Create module HomeHero.astro — props only; render minimal hero.
3. Create index.astro — call getHomepage in frontmatter; pass props to HomeHero.
