...
**Description:**
Implement the `WorkRecord` block module mapped by `_modelApiKey`.

**Outcome we expect:**
The `WorkRecord` block renders with props only and matches design intent.
...
**Legacy component:** `2022-site/gql/fragments/inline-blocks.gql.js (onWorkRecord)`

**Design system:** Use `@rotate/design-system` tokens for typography, spacing, and layout.

**Figma:** TODO — add Figma URL for `Inline Block — Work`.

## 3. Delivery Plan (how)

**Steps:**
1. Create module component for `WorkRecord`.
...
| # | Criterion | Done |
|---|---|---|
| AC1 | `WorkRecord` module renders without data fetching. | |
...
- Legacy: `2022-site/gql/fragments/inline-blocks.gql.js (onWorkRecord)`

## 6. Notes

- Inline link to work record (slug + title).
