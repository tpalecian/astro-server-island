...
**Separate plan.** Do not merge with the CMS service pattern plan ([technical/cms-service-pattern-and-dato-centralisation.md](../technical/cms-service-pattern-and-dato-centralisation.md)). This doc covers only the phased migration of 2022-site to Astro (routes, components, design-system). CMS/data access follows the other plan.

**Status:** Draft / in progress. When approved, move to `documentation/technical/` or `documentation/overview/`.  
**Format:** STL (Summary → Context → Technical/execution). See [.cursor/rules/planning.mdc](../../.cursor/rules/planning.mdc).
...
- **Data layer:** `packages/service-dato`; see [documentation/technical/dato-vue-to-service-review.md](../technical/dato-vue-to-service-review.md)

### Open questions / TBD

- Exact redirect list (old URLs → new) and where to configure (e.g. Vercel).
- Whether styleguide routes move as-is or are simplified.
